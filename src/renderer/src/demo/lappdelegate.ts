...
export class LAppDelegate {
   ...
   * APPに必要な物を初期化する。
   */
  public initialize(canvasRef: HTMLCanvasElement): boolean {
    // キャンバスの作成
  ...
  public createShader(): WebGLProgram {
    ...
    // プログラムオブジェクトの作成
    const programId = gl.createProgram() as any;
  ...
  public initializeCubism(): void {
    ...
    this._cubismOption.loggingLevel = window.log.CUBISM_LOGGING_LEVEL;
  ...
  private _resizeCanvas(): void {
    ...
    canvas.width = parseInt(canvas.style.width) || 240
    canvas.height = parseInt(canvas.style.height) || 480
  }
