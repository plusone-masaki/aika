...
export class LAppLive2DManager {
  ...
  public onTap(x: number, y: number): void {
    if (window.log.DEBUG_LOG_ENABLE) {
    ...
    for (let i = 0; i < this._models.getSize(); i++) {
      if (this._models.at(i).hitTest(window.model.HIT_AREA_NAME_HEAD, x, y)) {
        if (window.log.DEBUG_LOG_ENABLE) {
          LAppPal.printMessage(
            `[APP]hit area: [${window.model.HIT_AREA_NAME_HEAD}]`
          );
        }
        this._models.at(i).setRandomExpression();
      } else if (this._models.at(i).hitTest(window.model.HIT_AREA_NAME_BODY, x, y)) {
        if (window.log.DEBUG_LOG_ENABLE) {
          LAppPal.printMessage(
            `[APP]hit area: [${window.model.HIT_AREA_NAME_BODY}]`
          );
        }
        this._models
          .at(i)
          .startRandomMotion(
            window.model.MOTION_GROUP_TAP_BODY,
            window.model.PRIORITY_NORMAL,
  ...
  public async changeScene(index: number): Promise<void> {
    ...
    const model: string = modelList[index];
