...
export class LAppView {
  ...
  public initialize(): void {
    ...
    const bottom: number = window.view.VIEW_LOGICAL_BOTTOM;
    const top: number = window.view.VIEW_LOGICAL_TOP;

    this._viewMatrix.setScreenRect(left, right, bottom, top); // デバイスに対応する画面の範囲。 Xの左端、Xの右端、Yの下端、Yの上端
    this._viewMatrix.scale(window.view.VIEW_SCALE, window.view.VIEW_SCALE);
    ...
    // 表示範囲の設定
    this._viewMatrix.setMaxScale(window.view.VIEW_MAX_SCALE); // 限界拡張率
    this._viewMatrix.setMinScale(window.view.VIEW_MIN_SCALE); // 限界縮小率

    // 表示できる最大範囲
    this._viewMatrix.setMaxScreenRect(
      window.view.VIEW_LOGICAL_MAX_LEFT,
      window.view.VIEW_LOGICAL_MAX_RIGHT,
      window.view.VIEW_LOGICAL_MAX_BOTTOM,
      window.view.VIEW_LOGICAL_MAX_TOP,
    );
  }
